// ElevenLabs HTTP TTS Integration Test
//
// Tests the ElevenLabs HTTP streaming TTS provider with real API calls.
// Synthesizes speech and saves the output as a WAV file.
//
// Usage:
//   go run tests/elevenlabs_http_tts/elevenlabs_http_tts_run.go
//
// Environment:
//   ELEVENLABS_API_KEY - Required ElevenLabs API key
//
// Output:
//   tests/audiofiles/elevenlabs_http_tts_streaming.wav (16kHz mono)
//   tests/audiofiles/elevenlabs_http_tts_batch.wav (16kHz mono)
...
func main() {
	...
	log.Println("=== ElevenLabs HTTP TTS Integration Test ===")
	log.Println()
	...
	// Create provider
	provider, err := tts.NewElevenLabsHTTPTTSProvider(tts.ElevenLabsHTTPTTSConfig{
		APIKey:  apiKey,
	...
	log.Printf("[OK] Created ElevenLabs HTTP TTS provider (voice: %s)", defaultVoiceID)

	// Test text
	testText := "Hello! This is a test of the ElevenLabs HTTP streaming text to speech API. " +
	...
	log.Println("=== Test 1: HTTP Streaming Synthesis ===")
	...
	log.Println()
	log.Println("=== Test 2: HTTP Batch Synthesis ===")
	batchAudio, err := testBatchSynthesis(ctx, provider, "Hello world, this is a batch test using HTTP streaming.")
	if err != nil {
	...
	streamingOutputPath := filepath.Join(outputDir, "elevenlabs_http_tts_streaming.wav")
	...
	batchOutputPath := filepath.Join(outputDir, "elevenlabs_http_tts_batch.wav")
	...
	log.Println()
	log.Println("[PASS] ElevenLabs HTTP TTS test completed successfully")
}

func testStreamingSynthesis(ctx context.Context, provider *tts.ElevenLabsHTTPTTSProvider, text string) ([]byte, error) {
	startTime := time.Now()
...
func testBatchSynthesis(ctx context.Context, provider *tts.ElevenLabsHTTPTTSProvider, text string) ([]byte, error) {
	startTime := time.Now()
